        ...
        public override string GetValue(object value)
        {
            if (!IsNumber(value))
            {
                throw new InvalidOperationException(
                    $"'{nameof(NumberToStringCultureAttribute)}' attribute can be used only on 'numeric' value type properties.");
            }

            return double.Parse(value.ToString()).ToString(this.culture);
        }
