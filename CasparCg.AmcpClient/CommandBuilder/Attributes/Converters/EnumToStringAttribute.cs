...
{
    /// <summary>
    /// Convert enum to string (Gets value from
        ...
        public override string GetValue(object value)
        {
            if (!(value is Enum))
            {
                throw new InvalidOperationException(
                    $"'{nameof(EnumToStringAttribute)}' attribute can be used only on 'Enum' type properties.");
            }
            ...
            if (fieldInfo == null)
            {
                throw new ArgumentException(
                    $"'{nameof(EnumToStringAttribute)}' attribute can not found value '{value}' in enum '{value.GetType().Name}'.");
            }
