...
namespace CasparCg.AmcpClient.CommandBuilder
{
    ...
    {
        private HashSet<Tuple<string, string, string>> checkedProperty;
        ...
        public string BuildString(object obj)
        {
            if (obj.GetType().GetCustomAttribute<CommandBuilderObjectAttribute>() == null)
            {
                throw new InvalidOperationException(
                    $"Cannot build string on '{obj.GetType().Name}' object without '{nameof(CommandBuilderObjectAttribute)}' attribute.");
            }
        ...
        private string BuildStringRecursive(object obj, int deepLevel)
        {
            ...
            foreach (PropertyInfo property in CommandParameterAttribute.GetOrderedProperties(obj))
            {
                // Get property value
                ...
                {
                    string value = BuildStringRecursive(propertyValue, deepLevel);
                ...
                CommandParameterAttribute commandParameterAttribute = property.GetCustomAttribute<CommandParameterAttribute>();
                ...
                string convertedPropertyValue = GetConvertedPropertyValue(obj, property);
        ...
        private string GetConvertedPropertyValue(object obj, PropertyInfo property)
        {
            ...
            foreach (AbstractValueToStringConverterAttribute converter in property
                .GetCustomAttributes<AbstractValueToStringConverterAttribute>())
            {
        ...
        {
            this.checkedProperty = new HashSet<Tuple<string, string, string>>();
        ...
        private bool CanIncludePropertyRecursive(object obj, PropertyInfo propertyInfo, int deepLevel)
        {
            ...
            foreach (AbstractPropertyIncludedConditionAttribute propertyAttribute in propertyInfo
                .GetCustomAttributes<AbstractPropertyIncludedConditionAttribute>())
            {
                // Track checked property-attributes and ignore it if already checked, to prevent recursive dead loop
                if (!this.checkedProperty.Add(new Tuple<string, string, string>(propertyInfo.Name,
                    propertyAttribute.GetType().Name, propertyAttribute.PropertyName)))
                {
                    continue;
                }

                PropertyInfo property = obj.GetType().GetProperty(propertyAttribute.PropertyName);

                if (property == null)
                {
                    throw new InvalidOperationException(
                        $"Property '{propertyAttribute.PropertyName}' requested by '{propertyAttribute.GetType().Name}' attribute on '{propertyInfo.Name}' property not found.");
                }

                if (property.Name.Equals(propertyInfo.Name))
                {
                    throw new InvalidOperationException(
                        $"Cannot use '{propertyAttribute.GetType().Name}' attribute on '{propertyInfo.Name}' property to check property it self.");
                }

                bool canIncludeProperty = CanIncludePropertyRecursive(obj, property, deepLevel);
            ...
            bool hasValueConditionAttribute = false;
            bool wasTrue = false;

            foreach (AbstractPropertyValueConditionAttribute propertyAttribute in propertyInfo
                .GetCustomAttributes<AbstractPropertyValueConditionAttribute>())
            {
            ...
            return !hasValueConditionAttribute || wasTrue;
        }
    }
}