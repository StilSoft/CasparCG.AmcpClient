...
namespace CasparCg.AmcpClient.Commands.Query.Common
{
    public class MediaFileInfo : CasparFileInfo
    {
        ...
        public MediaFileInfo(string fullName)
            : base(fullName)
        {
        }

        public MediaFileInfo(string fileName, string filePath)
        ...
        public static MediaFileInfo Parse(string data)
        {
            ...
            try
            {
                int fullNameStartIndex = data.IndexOf('\"');
                int fullNameEndIndex = data.LastIndexOf('\"');
                string fullName = data.Substring(fullNameStartIndex + 1, fullNameEndIndex - 1);
                string remainData = data.Substring(fullNameEndIndex + 1);
                string[] values = remainData.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                ...
                long size = long.Parse(values[1]);
                var lastUpdate = DateTime.ParseExact(values[2], "yyyyMMddHHmmss", CultureInfo.InvariantCulture,
                    DateTimeStyles.None);
                ...
                if (mediaType == MediaType.Movie)
                {
                    ...
                    string[] fpsValues = values[4].Split('/');
                    double numerator = double.Parse(fpsValues[0]);
                    double denumerator = double.Parse(fpsValues[1]);
